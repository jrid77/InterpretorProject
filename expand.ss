  ...
  (lambda (exp)
    (cases expression exp
	   [lambda-exp (declaration body)
		       (lambda-exp declaration
				   (map syntax-expand body))]
	   [lambda-exp-one-var (declaration body)
			       (lambda-exp-one-var declaration
						   (map syntax-expand body))]
	   [lambda-exp-improper-list (declaration body)
				     (lambda-exp-improper-list declaration
							       (map syntax-expand body))]
	   [let-exp (declaration body) 
		    (syntax-expand
		     (app-exp (lambda-exp (map extract-let-vars declaration) body) 
			      (map extract-let-bindings declaration)))]
	   [let*-exp (declarations bodies)
		     (syntax-expand
		      (if (null? declarations)
			  (let-exp '() bodies)
			  (let-exp (list (car declarations))
				   (list (let*-exp (cdr declarations) bodies)))))]
	   ...
	   [case-exp (id keys bodies)
			 ...
			 (syntax-expand 
			  (if-else-exp (app-exp (var-exp 'member)
				       ...
				       (1st bodies)
				       (case-exp id (cdr keys) (cdr bodies)))))]
	   [app-exp (rator rands)
...
(define make-set!-exps
  (lambda (vars bindings)
    (if (null? vars)
	'()
	(cons (set!-exp (1st vars) (1st bindings))
